...
def load_data():
    ...
    df_time['Date'] = [date.replace("Sept", "Sep") for date in df_time['Date']]
...
median_agg = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo].iloc[:, 3:].median()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    metric_medians6mo = metric_medians6mo[1:]
    metric_medians12mo = metric_medians12mo[1:]
    ...
    st.dataframe(df_agg_diff_final.style.hide().applymap(style_negative, props='color:red;').applymap(style_positive, props='color:green;').format(
        df_to_pct))
